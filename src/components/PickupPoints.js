  ...
  return (
    <section className="py-16" id="puntos">
      <div className="container mx-auto px-4">
        <h2 className="text-2xl font-bold text-gray-900 mb-8 text-center">Puntos de recogida y entrega</h2>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {pickupPoints.map((point) => (
            <div 
              key={point.id} 
              className={`bg-white p-4 rounded-lg shadow-md border-l-4 ${selectedPoint?.id === point.id ? 'border-green-500' : 'border-gray-200'} cursor-pointer hover:shadow-lg transition-shadow`}
            ...
            >
              ...
              <p className="text-gray-700 text-sm mt-2"><span className="font-medium">Horario:</span> {point.hours}</p>
              <p className="text-green-600 text-xs mt-2">
                Zonas de entrega gratis (8AM-12M): {point.deliveryZones.join(', ')}
              </p>
            </div>
          ))}
        </div>
        ...
        {selectedPoint && (
          <div className="bg-green-50 p-6 rounded-lg mb-8">
            <h3 className="text-xl font-semibold text-green-800 mb-4">Opciones de entrega a domicilio</h3>
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Horario de entrega</label>
              <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-2 sm:space-y-0">
                <label className="inline-flex items-center">
                  ...
                  />
                  <span className="ml-2">8:00 AM - 12:00 M ({selectedPoint.deliveryZones.includes(deliveryAddress.split(',')[0]?.trim()) ? 'GRATIS' : `+${selectedPoint.deliveryCost.morning} CUP`})</span>
                </label>
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    ...
                    className="text-green-600 focus:ring-green-500"
                  />
                  <span className="ml-2">12:30 PM - 5:00 PM (+{selectedPoint.deliveryCost.afternoon} CUP)</span>
                </label>
              </div>
            </div>
            ...
            {deliveryAddress && (
              <div className="bg-white p-4 rounded-md shadow-sm">
                <h4 className="font-medium text-gray-800">Resumen de entrega</h4>
                <p className="text-gray-600 mt-2">Costo de envío: {calculateDeliveryCost()} CUP</p>
                <p className="text-gray-600">Dirección: {deliveryAddress}</p>
                <p className="text-gray-600">Punto seleccionado: {selectedPoint.name}</p>
                <button className="mt-4 px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors w-full sm:w-auto">
                  Confirmar entrega
                </button>
              </div>
            )}
        ...
        <div className="bg-blue-50 p-6 rounded-lg">
          <h3 className="text-xl font-semibold text-blue-800 mb-4">¿Cómo funciona?</h3>
